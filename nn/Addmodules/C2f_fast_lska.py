...
class CSPPC_LSKA_Bottleneck(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.DualPConv = nn.Sequential(Partial_conv3(dim, n_div=4, forward='split_cat'),
                                       LSKA(1),
                                       Partial_conv3(dim, n_div=4, forward='split_cat'))
        self.LSKAttention = LSKA(1)
    def forward(self, x):
        return self.DualPConv(x)

class C2f_FAST_LSKA(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.m = nn.ModuleList(CSPPC_LSKA_Bottleneck(self.c) for _ in range(n))
